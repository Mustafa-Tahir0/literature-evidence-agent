...
python -m py_compile *.py                                            # syntax (run from inside the repo dir)
```

> **Known issue:** the commands this section previously listed
> (`python -m pytest agent_skeleton/tests -q`, `python -m py_compile agent_skeleton/*.py`)
> don't work on a fresh clone: the literal path `agent_skeleton/...` doesn't
> exist on disk (the folder is `agent-skeleton`, hyphenated — see "Import name"
> in Gotchas below), and even after fixing the path, `pytest` itself fails
> every test with `ImportError: attempted relative import with no known parent
> package`. Root cause: pytest creates a `Package` collector for any directory
> containing `__init__.py`, including this repo's root (since `package-dir`
> maps `agent_skeleton` to `.`); importing that `Package` node requires a valid
> dotted module name, which pytest can't construct because the on-disk
> directory name has a hyphen. This is 100% reproducible, independent of OS,
> `--rootdir`, or `--import-mode`. Every test file's `python -m <module>`
> invocation (used above) sidesteps pytest's collection entirely and passes
> the same test functions; a real fix would need a `src/agent_skeleton/`-style
> layout so the repo root itself doesn't contain `__init__.py`.
