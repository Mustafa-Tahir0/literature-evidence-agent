...
def test_default_demo_spec_backward_compatible():
    # This agent replaced the stock word_count/reverse_text demo tools in
    # tool_schemas.py/tools.py with its own (network-backed) literature search
    # tools, per the template's own Path-A instructions. default_demo_spec()
    # reflects whatever is currently registered, so this checks structural
    # wiring (schemas <-> functions still align, dispatch still works) rather
    # than hardcoding the original demo tool names or making a live network call.
    demo = default_demo_spec()
    demo.validate()
    assert set(demo.tool_registry) == {"search_pubmed", "search_semantic_scholar", "search_arxiv"}
    assert demo.dispatch("missing_tool", {})["ok"] is False
